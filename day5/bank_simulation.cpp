             ...
             int win_count, EventHandler handlers[], StatsCollector *stats)
{
    ...
    while (running && !event_list_empty(el))
    {
        ...
        // 每次事件处理后检查过号
        if (g_current_time >= cfg->open_time + cfg->no_response_timeout)
        {
            scheduler_check_no_response(cfg, queue, el, g_current_time);
        }
    }
...
void handle_arrival(Event *e, SimConfig *cfg, Queue *queue, Window *wins, EventList *el, StatsCollector *stats)
{
    ...
    // 设置优先级权重
    if (c->client_type == VIP_CLIENT) c->priority_weight = 3;
    else if (c->client_type == CORPORATE) c->priority_weight = 2;
    else if (c->client_type == ELDERLY) c->priority_weight = 2;
    else c->priority_weight = 1;

    c->is_appointment = (rng_uniform(cfg->rng, 0, 1) < 0.1) ? 1 : 0;
    ...
    // 检查空闲窗口，有则立即叫号
    Window *idle_win = window_find_idle(wins, cfg->window_count, WIN_INDIVIDUAL);
    if (idle_win)
    {
        Client *svc = queue_dequeue(queue, min_id);
        if (svc)
        {
            svc->status = STATUS_IN_SERVICE;
            svc->start_time = g_current_time;
            idle_win->status = WIN_SERVING;
            idle_win->current_client = svc;
            stats_record_service_start(stats, svc, idle_win->id);

            // 计算服务时长
            BizDurationParam bp = cfg->biz_duration[svc->business];
            double duration = rng_lognormal(cfg->rng, bp.mu, bp.sigma) * idle_win->staff_k;
            Event end_evt;
            end_evt.timestamp = g_current_time + duration;
            end_evt.type = END_SERVICE;
            end_evt.data.client = svc;
...
void handle_bulk_arrival(Event *e, SimConfig *cfg, Queue *queue, Window *wins, EventList *el, StatsCollector *stats)
{
    ...
    for (int i = 0; i < count; i++)
    {
        ...
        c->end_time = -1;
        c->queue_id = -1;
        c->priority_weight = (c->client_type == VIP_CLIENT) ? 3 : 1;
        c->is_appointment = 0;
    ...
    // 安排下一次突发
    Event next_bulk;
    next_bulk.timestamp = g_current_time + rng_uniform(cfg->rng, 30, 90);
    next_bulk.type = BULK_ARRIVAL;
    next_bulk.data.client = NULL;
    if (next_bulk.timestamp < cfg->close_stop_time)
        event_list_insert(el, next_bulk);
}
...
void handle_end_service(Event *e, SimConfig *cfg, Queue *queue, Window *wins, EventList *el, StatsCollector *stats)
{
    ...
    for (int i = 0; i < cfg->window_count; i++)
    {
        if (wins[i].current_client == c)
        {
            ...
            double duration = c->end_time - c->start_time;
            wins[i].total_busy_time += duration;
            wins[i].total_served++;
            wins[i].status = WIN_IDLE;
            wins[i].current_client = NULL;

            stats_record_service_end(stats, c, wins[i].id, duration);
            stats_record_wait(stats, c, c->start_time - c->arrival_time);
            stats_record_satisfaction(stats, c, cfg->max_satisfy_wait);
            free(c);

            // 异常中断检查
            if (rng_uniform(cfg->rng, 0, 1) < cfg->error_probability)
            {
                wins[i].total_errors++;
                stats_record_error(stats, wins[i].id);
                wins[i].status = WIN_SWITCHING;
                Event switch_evt;
                switch_evt.timestamp = g_current_time + 2.0;
                switch_evt.type = WINDOW_SWITCH;
                switch_evt.data.window_id = wins[i].id;
            ...
            else
            {
                ...
                int min_id;
                queue_get_min_length(queue, &min_id);
                Client *next = queue_dequeue(queue, min_id);
                if (next)
                {
                    ...
                    stats_record_service_start(stats, next, wins[i].id);

                    BizDurationParam bp = cfg->biz_duration[next->business];
                    double dur = rng_lognormal(cfg->rng, bp.mu, bp.sigma) * wins[i].staff_k;
                    Event end_evt;
                    end_evt.timestamp = g_current_time + dur;
                    end_evt.type = END_SERVICE;
                    end_evt.data.client = next;
                    event_list_insert(el, end_evt);
                }
            }
            break;
...
void handle_window_switch(Event *e, SimConfig *cfg, Queue *queue, Window *wins, EventList *el, StatsCollector *stats)
{
    ...
    {
        wins[wid].status = WIN_IDLE;
        wins[wid].type = WIN_INDIVIDUAL;
    }
}
...
void handle_shutdown(Event *e, SimConfig *cfg, Queue *queue, Window *wins, EventList *el, StatsCollector *stats)
{
    ...
    for (int i = 0; i < cfg->window_count; i++)
    {
        ...
        for (int j = 0; j < len; j++)
        {
            Client *c = queue_dequeue(queue, i);
            if (c)
            {
...
void scheduler_check_no_response(SimConfig *cfg, Queue *queue, EventList *el, double current_time)
{
    Client *no_resp = queue_find_no_response(queue, current_time, cfg->no_response_timeout);
    if (no_resp)
    {
        // 从队列移除，降级重新入队
        Client *c = queue_remove_by_id(queue, no_resp->queue_id, no_resp->id);
        if (c)
        {
            c->is_appointment = 0;
            c->priority_weight = 1;
...
// ====================== P4 占位实现 ======================
// ==========================================================================
void stats_record_arrival(StatsCollector *stats, Client *c) { stats->total_arrivals++; }
void stats_record_service_start(StatsCollector *stats, Client *c, int window_id) {}
void stats_record_service_end(StatsCollector *stats, Client *c, int window_id, double duration) { stats->total_completed++; }
void stats_record_wait(StatsCollector *stats, Client *c, double wait_time) { stats->wait_time_sum[c->client_type] += wait_time; stats->wait_time_count[c->client_type]++; }
void stats_record_balk(StatsCollector *stats, Client *c) { stats->total_balked++; }
void stats_record_jockey(StatsCollector *stats, Client *c, int from_q, int to_q) { stats->total_jockeyed++; }
void stats_record_error(StatsCollector *stats, int window_id) { stats->total_errors++; if(window_id < MAX_WINDOWS) stats->window_errors[window_id]++; }
void stats_record_queue_length(StatsCollector *stats, int length) { if(length > stats->max_queue_length) { stats->max_queue_length = length; stats->max_queue_time = g_current_time; } }
void stats_print_event(double time, const char *category, const char *fmt, ...) { char buf[20]; printf("[%s][%s] ", time_format(time, buf, 20), category); va_list args; va_start(args, fmt); vprintf(fmt, args); va_end(args); printf("\n"); }
double stats_get_max_queue_length(const StatsCollector *stats) { return stats->max_queue_length; }
void stats_record_satisfaction(StatsCollector *stats, Client *c, double max_possible_wait) { double wait = (c->start_time > 0) ? c->start_time - c->arrival_time : max_possible_wait; double sat = (wait < max_possible_wait) ? 1.0 - wait / max_possible_wait : 0.0; stats->satisfaction_sum += sat; stats->satisfaction_count++; }
void stats_write_report(const StatsCollector *stats, const char *filename, const Window *wins, int win_count) { FILE *f = fopen(filename, "w"); if(!f) return; fprintf(f, "=====银行排队仿真报告=====\n"); fprintf(f, "总到达: %d\n总完成: %d\n总弃号: %d\n", stats->total_arrivals, stats->total_completed, stats->total_balked); fprintf(f, "最大排队长度: %d (时刻 %.1f)\n", stats->max_queue_length, stats->max_queue_time); double avg_sat = (stats->satisfaction_count > 0) ? stats->satisfaction_sum / stats->satisfaction_count : 0; fprintf(f, "平均满意度: %.2f%%\n", avg_sat * 100); fclose(f); }
void log_init(const char *filename) {}
void log_close(void) {}
void log_event(double time, const char *category, const char *fmt, ...) {}
void log_printf(const char *fmt, ...) { va_list args; va_start(args, fmt); vprintf(fmt, args); va_end(args); }
...
int main(int argc, char *argv[])
{
    ...
    cfg.close_force_time = 60;  // 测试用：只跑60分钟
    cfg.close_stop_time = 50;   // 停止取号时间
    rng_init(&rng, 12345);
    cfg.rng = &rng;
